...
import { stack, stackOffsetExpand, type stackOffsetNone } from 'd3-shape'
...
export function createYScale(
    ...
    if (range.count === 0) {
        return scaleLinear().domain([0, 1]).range([dimensions.plotTop + dimensions.plotHeight, dimensions.plotTop])
    }
...
function buildStackData(
    ...
    for (const axisSeries of seriesByAxis.values()) {
        ...
        const stackGen = stack<Record<string, number>>().keys(axisSeries.map((s) => s.key))
        if (offset) {
