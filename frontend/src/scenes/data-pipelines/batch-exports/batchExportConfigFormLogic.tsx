...
export function getDefaultConfiguration(service: string): Record<string, any> {
    ...
    return {
        ...
        model: 'events',
        paused: true,
        ...(definition ? definition.defaults({ featureFlags: {} as any }) : {}),
    }
}
...
export const batchExportConfigFormLogic = kea<batchExportConfigFormLogicType>([
    ...
    listeners(({ props, values, actions }) => ({
        ...
        setConfigurationValue: async ({ name, value }) => {
            ...
            // Destination-specific side effects (e.g. BigQuery's JSON config file upload).
            const destination = values.configuration.destination as BatchExportService['type'] | undefined
            const definition = destination ? DESTINATIONS[destination] : undefined
            if (definition?.onFieldChange) {
                await definition.onFieldChange(fieldName as string, value, {
                    current: values.configuration,
                    setValues: (vals) => actions.setConfigurationValues(vals),
                    setManualErrors: (errors) => actions.setConfigurationManualErrors(errors),
                })
            }
