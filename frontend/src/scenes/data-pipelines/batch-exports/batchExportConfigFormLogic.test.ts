...
// it as a form-only field and is dropped on save.
...
describe('batchExportConfigFormLogic', () => {
    ...
    describe('required fields validation', () => {
        // Each destination's required-field declaration in DESTINATIONS is the source of truth;
        // this test verifies the form pipeline propagates it into form errors for an empty submit.
        it.each(Object.keys(DESTINATIONS) as (keyof typeof DESTINATIONS)[])(
            'rejects empty form for %s',
            async (service) => {
                ...
                const expectedFields = [
                    ...
                    'name',
                    'model',
                    ...DESTINATIONS[service].requiredFields({
                        isNew: true,
                        formValues: getDefaultConfiguration(service),
                        featureFlags: {},
                    }),
                ]
    ...
    describe('Redshift bucket validation only runs in COPY mode', () => {
        // Validates that Redshift's mode-conditional validate hook only kicks in for COPY mode.
    ...
    describe('BigQuery onFieldChange uploads parsed JSON config', () => {
        // The registry hook reads json_config_file via FileReader, parses JSON, and lifts the
    ...
    describe('successful update', () => {
        it('sends changed field plus untouched fixture defaults', async () => {
            ...
            // Derived from the fixture so registry default changes don't break this test.
    ...
    describe('create with required fields per destination', () => {
        it.each([
            ...
            },
            {
                ...
                // we set parsed fields directly to bypass the onFieldChange file-reader side effect.
