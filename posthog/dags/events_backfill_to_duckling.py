...
def ensure_iceberg_table_exists(
    ...
    try:
        ...
        conn.execute(ddl.format(catalog=ICEBERG_ALIAS))
        return True
    except Exception as exc:
        context.log.warning(
            f"Iceberg dual-write disabled for this run — could not ensure {ICEBERG_ALIAS}.posthog.{table}: {exc}. "
            "Is the Iceberg backend enabled for this org's warehouse?"
        )
        logger.warning(
...
def write_partition_to_iceberg(
    ...
    try:
        if partition_date is None:
            with conn.cursor() as cur:
                cur.execute(f"DELETE FROM {ICEBERG_ALIAS}.posthog.{table} WHERE team_id = %s", (team_id,))
        else:
            date_str = partition_date.strftime("%Y-%m-%d")
            next_date_str = (partition_date + timedelta(days=1)).strftime("%Y-%m-%d")
            with conn.cursor() as cur:
                cur.execute(
                    f"DELETE FROM {ICEBERG_ALIAS}.posthog.{table} "
                    f"WHERE team_id = %s AND {timestamp_column} >= %s AND {timestamp_column} < %s",
                    (team_id, date_str, next_date_str),
                )
    except Exception as exc:
        context.log.warning(
    ...
    try:
        if conn is not None:
            ...
            if iceberg_enabled_for_team(team_id):
                ...
                iceberg_enabled = ensure_iceberg_table_exists(context, conn, "events", EVENTS_TABLE_DDL)
    ...
    try:
        if conn is not None:
            ...
            if iceberg_enabled_for_team(team_id):
                ...
                iceberg_enabled = ensure_iceberg_table_exists(context, conn, "persons", ICEBERG_PERSONS_TABLE_DDL)
