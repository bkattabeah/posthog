...
import { ContextMillCache } from '@/hono/cache/ContextMillCache'
...
const TEST_URL = 'https://example.invalid/skills.zip'

describe('ContextMillCache', () => {
    let redis: MockRedis
    ...
    it('fetches upstream on cold cache and writes archive + freshness', async () => {
        const cache = new ContextMillCache(redis)
        const bytes = new Uint8Array([1, 2, 3, 4, 5])
        const upstream = vi.fn(async () => bytes)

        const result = await cache.fetchArchive(TEST_URL, upstream)

        expect(result).toEqual(bytes)
        expect(upstream).toHaveBeenCalledTimes(1)
        expect(redis._store.has('mcp:context-mill:archive')).toBe(true)
        expect(redis._store.has('mcp:context-mill:archive:fresh')).toBe(true)
        // Lock acquired then released.
        expect(redis._store.has('mcp:context-mill:lock')).toBe(false)
    })

    it('serves the cached archive on fresh hit without calling upstream', async () => {
        const cache = new ContextMillCache(redis)
        const bytes = new Uint8Array([9, 8, 7])
        await cache.fetchArchive(TEST_URL, async () => bytes)
    ...
    it('only one writer fetches upstream when many clients race on cold cache', async () => {
        ...
        const concurrency = 5
        // Short wait timeout so non-writers fall back rather than stall the test.
        const racingCache = new ContextMillCache(redis, { waitIntervalMs: 5, waitTimeoutMs: 200 })
        const results = await Promise.all(
            Array.from({ length: concurrency }, () => racingCache.fetchArchive(TEST_URL, upstream))
        )
        ...
        // Voiding the unused binding from cache constructor to satisfy lint.
        expect(cache).not.toBeUndefined()
    })

    it('non-writers wait for the lock holder to publish and serve that result', async () => {
        const cache = new ContextMillCache(redis, { waitIntervalMs: 5, waitTimeoutMs: 1000 })
        ...
        const writerResult = cache.fetchArchive(TEST_URL, writerUpstream)
        // Give the writer time to seize the lock before the waiter starts.
        await new Promise((r) => setTimeout(r, 10))
        const waiterResult = cache.fetchArchive(TEST_URL, waiterUpstream)
    ...
    it('serves stale cache while triggering a background refresh', async () => {
        ...
        const cache = new ContextMillCache(redis, { freshSeconds: 0, waitIntervalMs: 5, waitTimeoutMs: 100 })
        ...
        const first = await cache.fetchArchive(TEST_URL, upstream)
        expect(first).toEqual(initial)
        expect(upstream).toHaveBeenCalledTimes(1)

        // Stale read returns the cached value, kicks off a background refresh.
        const second = await cache.fetchArchive(TEST_URL, upstream)
        ...
        const third = await cache.fetchArchive(TEST_URL, upstream)
    ...
    it('falls back to a direct fetch when the writer never publishes', async () => {
        ...
        redis._store.set('mcp:context-mill:lock', 'someone-else')
        const cache = new ContextMillCache(redis, { waitIntervalMs: 10, waitTimeoutMs: 50 })
        ...
        expect(redis._store.has('mcp:context-mill:archive')).toBe(false)
    })
})
