...
class TestEndpointViewSetPSAKAuth(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_psak_can_run_endpoint_with_hogql_access_control_on(self):
        ...
        with (
            ...
            patch("posthog.hogql.database.database.Database._filter_system_tables_for_user") as user_path,
            patch("posthog.hogql.database.database.Database._filter_all_scoped_system_tables") as anon_path,
        ):
            response = self.client.post(
                f"/api/projects/{self.team.id}/endpoints/my_endpoint/run/",
                data={},
        ...
        self.assertGreater(anon_path.call_count, 0)
